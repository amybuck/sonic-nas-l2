...
#define NAS_STG_LOG(type, lvl , msg, ...)\
                       EV_LOG(type,NAS_L2,lvl,"NAS-STG",msg, ##__VA_ARGS__)
...
typedef std::unordered_map <hal_ifindex_t, nas_lag_port_list_t> nas_stg_lag_map_t;
 ...
 */
typedef std::unordered_set <hal_ifindex_t> nas_stg_port_list_t;

/*
 * vlan list to be used in the stg entry
 */
typedef std::unordered_set<hal_vlan_id_t> nas_stg_vlan_list_t;
typedef std::unordered_set<hal_vlan_id_t>::iterator nas_stg_vlan_list_it;


/*
 * Map to maintain list of npu id to stg id mapping
 */
typedef std::unordered_map<npu_id_t,ndi_stg_id_t> npu_to_stg_map_t;
typedef std::unordered_map<npu_id_t,ndi_stg_id_t>::iterator npu_to_stg_map_it;
 ...
 */
typedef std::unordered_map < nas_stg_id_t , nas_stg_entry_t> nas_stg_table_t;
typedef std::unordered_map <nas_stg_id_t , nas_stg_entry_t >::iterator nas_stg_table_it;
 ...
 */
typedef std::unordered_map < hal_ifindex_t , nas_stg_id_t > nas_br_to_stg_map_t;
typedef std::unordered_map < hal_ifindex_t , nas_stg_id_t >::iterator nas_br_to_stg_map_it;
 ...
 */
typedef std::unordered_map < hal_ifindex_t , hal_vlan_id_t > nas_br_to_vlan_map_t;
typedef std::unordered_map < hal_ifindex_t , hal_vlan_id_t >::iterator nas_br_to_vlan_map_it;
 ...
 */
typedef std::unordered_map < hal_vlan_id_t , nas_stg_id_t > nas_vlan_to_stg_map_t;
 ...
 */
typedef std::unordered_set<npu_id_t> nas_stg_npu_ids;


/*
 * Map which maintains switch id to list of NPUs
 */
typedef std::unordered_map<nas_switch_id_t,nas_stg_npu_ids>nas_stg_switch_npu_map_t;
 ...
 */
typedef std::unordered_map<nas_switch_id_t,nas_stg_id_t>nas_stg_switch_defult_stg_map_t;
