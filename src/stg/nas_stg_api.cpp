...
// Map which maintains the lag to stp state mapping
static std::unordered_map<hal_ifindex_t,BASE_STG_INTERFACE_STATE_t> lag_stp_state_map;

// Map which maintains the lag to stg id mapping
static std::unordered_map<hal_ifindex_t,nas_stg_id_t> lag_to_stp_map;
...
static bool nas_stg_intf_to_port(hal_ifindex_t ifindex, interface_ctrl_t *intf_ctrl) {
    ...
    if (dn_hal_get_interface_info(intf_ctrl) != STD_ERR_OK) {
        NAS_STG_LOG(ERR, 0, "Interface %d has NO slot %d, port %d", intf_ctrl->if_index,
...
static bool nas_update_stp_state(nas_stg_entry_t * entry, hal_ifindex_t ifindex,
    ...
    if( it == nas_lag_map.end()){
        ...
        members = &(it->second);
        lag_stp_state_map[ifindex] = state;
        // If LAG to STP mapping is not there then add the mapping
        if(lag_to_stp_map.find(ifindex) == lag_to_stp_map.end()){
            lag_to_stp_map[ifindex] = entry->nas_stg_id;
        }
    }

    if(members == nullptr){
        NAS_STG_LOG(ERR,0,"Member port list is null");
        return false;
    }
    // Iterate through the member in case of LAG
    for(auto intf_it = members->begin(); intf_it != members->end(); ++intf_it){
        NAS_STG_LOG(INFO,3,"Updating STP State for ifindex %d",*intf_it);
        ...
        NAS_STG_LOG(INFO, 3, "New STP State %d, Existing STP State %d", state, stp_state);
        ...
        if ((stp_state == BASE_STG_INTERFACE_STATE_BLOCKING)
            ...
            NAS_STG_LOG(INFO, 0, "Already Updated STP state to %d for interface %d",
                        state, ifindex);
            continue;
        }
        ...
        NAS_STG_LOG(INFO, 3, "Updated the STP state to %d for interface index %d in " "STG Id %d",
...
bool nas_create_stg_for_vlan(hal_vlan_id_t id,hal_ifindex_t bid){
    ...
    if (npu_it == switch_to_npu_map.end()) {
        NAS_STG_LOG(ERR, 0, "No such switch ID %d exist", entry.switch_id);
        return false;
    }
    ...
    NAS_STG_LOG(INFO, 3, "New STG Entry with id %d created", entry.nas_stg_id);
    return true;
}
        ...
        if(sit != vlan_to_stg_map.end()){
            ...
            if (entry_it != nas_stg_table.end()){
                ...
                if(entry->vlan_list.size()>1){
                    NAS_STG_LOG(INFO,3,"Create new STG instance for vlan %d",sit->first);
...
static bool nas_stg_update_os_stp_state(hal_ifindex_t intfindex,
    ...
    if (obj == NULL) {
        NAS_STG_LOG(ERR, 0, "Failed to create a new object");
        return false;
    }
    ...
    if(it == to_os_stp_state.end()){
        NAS_STG_LOG(ERR,0,"Invalid STP State %d to update os",state);
        return false;
    }
    cps_api_object_attr_add_u32(obj, BASE_STG_ENTRY_INTF_STATE, it->second);

    /* vlan id is needed to get the vlanned interface index to pass it to the kernel
     * default entry will always have untagged ports and in that case vlan ID is not needed
     */
    if (!default_entry) {
    ...
    if (nl_int_update_stp_state(obj) != STD_ERR_OK) {
        NAS_STG_LOG(ERR, 0, "Failed to updated kernel STP state to %d for interface index %d",
                state, intfindex);
        return true;
    }

    NAS_STG_LOG(INFO, 3, "Updated the kerenl STP State to %d for interface index  %d", state,
            intfindex);
    return true;
}
...
static bool nas_stg_get_default_ndi_id(nas_switch_id_t sid, npu_id_t npu_id, ndi_stg_id_t *ndi_id){
    ...
    if (it == switch_to_default_stg_map.end()) {
        NAS_STG_LOG(ERR, 0, "No switch id %d exist", sid);
        return false;
    }

    auto sit = nas_stg_table.find(it->second);
    if (sit == nas_stg_table.end()) {
        NAS_STG_LOG(ERR, 0, "No Entry id %d exist", it->second);
        return false;
    }
    ...
    if (nit == entry.npu_to_stg_map.end()) {
        NAS_STG_LOG(ERR, 0, "No Npu id %d exist", npu_id);
        return false;
    }
...
t_std_error nas_stg_update_stg_state(hal_ifindex_t bid, hal_ifindex_t intfindex,
    ...
    if( it == from_os_stp_state.end()){
        NAS_STG_LOG(ERR,0,"Invalid STP State received %d",os_state);
    ...
    if (vit != bridge_to_vlan_map.end()) {
        ...
        if (sit != vlan_to_stg_map.end()) {
            ...
            if (stg_table_it == nas_stg_table.end()) {
                NAS_STG_LOG(ERR, 0, "No STG Entry with bridge index %d and ID %d exist", bid,
                        sit->second);
    ...
    if((entry->vlan_list.size()>1) || (entry->cps_created == true)){
        NAS_STG_LOG(INFO, 3, "Skipping kernel update as stg instance has more "
...
static bool nas_stg_update_intf_info(nas_stg_entry_t * entry, cps_api_object_t obj,
    ...
    for (cps_api_object_it_inside (&it_lvl_1); cps_api_object_it_valid (&it_lvl_1);
        ...
        if (ifindex_attr == NULL || stp_state_attr == NULL) {
            NAS_STG_LOG(ERR, 0, "Missing Necessary parameters for setting interface STP state");
            return false;
        }
...
t_std_error nas_stg_delete_session(nas_stg_id_t id) {
    ...
    NAS_STG_LOG(INFO,3,"Deleting STG instance %d",id);

    if(default_entry->nas_stg_id == id){
        NAS_STG_LOG(ERR,0,"Cannot deleted default STG instance");
        return STD_ERR(STG,FAIL,0);
    }
    ...
    if (table_it != nas_stg_table.end()) {
        ...
        for (auto vit = entry.vlan_list.begin(); vit != entry.vlan_list.end(); ++vit) {
            if(!nas_stg_delete_vlan(&entry, *vit)){
                NAS_STG_LOG(ERR,0,"Failed to delete STG id %d",id);
                return STD_ERR(STG,FAIL,0);
            }
        }
        ...
        NAS_STG_LOG(INFO, 3, "Deleted the STG Instance with ID %d", entry.nas_stg_id);
        nas_stg_remove_index(entry.nas_stg_id);
        nas_stg_table.erase(table_it);

    } else {
        NAS_STG_LOG(ERR, ev_log_s_MINOR, "No STG instance with Id %d exist", id);
        return STD_ERR(STG, NEXIST, 0);
    }
...
static bool nas_stg_add_vlan(nas_stg_entry_t * entry, hal_vlan_id_t vlan_id){
   ...
   if (vlan_to_stg_it != vlan_to_stg_map.end()){
       ...
       if(stg_table_it == nas_stg_table.end()){
           NAS_STG_LOG(ERR,0,"No STG entry %d exist",vlan_to_stg_it->second);
           return false;
       }
...
static bool nas_stg_update_vlan_list(nas_stg_entry_t * entry, nas_stg_vlan_list_t & vlan_list) {
    ...
    for (auto it = entry->vlan_list.begin(); it != entry->vlan_list.end(); ++it) {
        if (vlan_list.find(*it) == vlan_list.end()) {
            if(!nas_stg_delete_vlan(entry,*it)){
                return false;
            }
            del_vlan_list.insert(*it);
        } else {
            vlan_list.erase(*it);
        }
    }
...
static bool nas_stg_update_entry(nas_stg_entry_t * entry, cps_api_object_t obj) {
    ...
    if (vlan_list_attr) {
        if (!nas_stg_update_vlan_list(entry, add_vlan_list)) {
            NAS_STG_LOG(ERR, 0, "Failed to update vlan list");
            return false;
        }
    }
    ...
    if (update_stp_state) {
        if (!nas_stg_update_intf_info(entry, obj, intf_it)) {
            NAS_STG_LOG(ERR, 0, "Error Updating STG interface state for STG instance %d",
                    entry->nas_stg_id);
            return false;
        }
    }
...
t_std_error nas_stg_cps_create_instance(cps_api_object_t obj) {
...

    {
        ...
        nas_stg_table.insert(nas_stg_table_pair(entry.nas_stg_id, std::move(entry)));
        NAS_STG_LOG(INFO, 3, "Created new STG Entry with id %d", entry.nas_stg_id);
    }
...
t_std_error nas_stg_set_instance(cps_api_object_t obj, nas_stg_id_t stg_id) {
    ...
    std_mutex_simple_lock_guard lock(&nas_stg_mutex);
    NAS_STG_LOG(INFO,3,"Updating STG id %d",stg_id);
    nas_stg_table_it it = nas_stg_table.find(stg_id);
    if (it == nas_stg_table.end()) {
        NAS_STG_LOG(ERR, 0, "No STG id %d exist", stg_id);
        return STD_ERR(STG, NEXIST, 0);
    }
...
t_std_error nas_stg_add_vlan_to_bridge(hal_ifindex_t bid, hal_vlan_id_t vlan_id) {
    ...
    if (it == bridge_to_vlan_map.end()) {
        ...
        NAS_STG_LOG(ERR, 0, "Adding Bridge %d to VLAN with id %d", bid,  bridge_to_vlan_map[bid]);
    } else {
        NAS_STG_LOG(ERR, 0, "Already vlan exist with bridge id %d", bid);
        return STD_ERR(STG, PARAM, 0);
    }
    return STD_ERR_OK;
}
...
t_std_error nas_stg_delete_instance(hal_ifindex_t bid) {
    ...
    if (it != bridge_to_stg_map.end()) {
        ...
        NAS_STG_LOG(ERR, 0, "No Bridge with id %d exist in the STG table", bid);
        return STD_ERR(STG, NEXIST, 0);
    }
...
t_std_error nas_stg_get_all_info(cps_api_object_list_t list) {
    ...
    for (nas_stg_table_it tit = nas_stg_table.begin(); tit != nas_stg_table.end(); ++tit) {
        ...
        if (obj == NULL) {
            NAS_STG_LOG(ERR, 0, "Failed to create/append new object to list");
...
t_std_error nas_stg_get_instance_info(cps_api_object_list_t list, nas_stg_id_t id,
    ...
    if (tit != nas_stg_table.end()) {
        ...
        NAS_STG_LOG(ERR, 0, "No STG instance %d exist", id);
        return STD_ERR(STG, NEXIST, 0);
    }
...
t_std_error nas_stg_create_default_instance() {
    ...
    if (npu_it == switch_to_npu_map.end()) {
        NAS_STG_LOG(ERR, 0, "No switch ID %d exist");
        return STD_ERR(STG, NEXIST, 0);
    }
    ...
    for (; npu_it != switch_to_npu_map.end(); ++npu_it) {
        ...
        vlan_to_stg_map[vlan_id] = entry.nas_stg_id;
        nas_stg_table.insert(nas_stg_table_pair(entry.nas_stg_id, std::move(entry)));
        NAS_STG_LOG(INFO, 3, "Created Default STG Instance with Id %d for switch id %d ",
...
t_std_error nas_stg_get_npu_list(void) {
    ...
    for (size_t ix = 0; ix < switches->number_of_switches; ++ix) {
        ...
        if (sd == NULL) {
            NAS_STG_LOG(ERR, 0, "Switch Details Configuration file is erroneous");
            return STD_ERR(STG, PARAM, 0);
        }
...
t_std_error nas_stg_get_default_instance(cps_api_object_list_t list){
    ...
    if (it == switch_to_default_stg_map.end()) {
        NAS_STG_LOG(ERR, 0, "No switch id %d exist", nas_switch_id);
        return STD_ERR(STG,NEXIST,0);
    }
...
t_std_error nas_stg_vlan_update(hal_vlan_id_t id,bool add,hal_ifindex_t bridge_index){
    ...
    std_mutex_simple_lock_guard lock(&nas_stg_mutex);
    if(add){
        NAS_STG_LOG(INFO,3,"Adding VLAN %d to Bridge %d",id,bridge_index);
        bridge_to_vlan_map[bridge_index]=id;
        auto it = vlan_to_stg_map.find(id);
        if( it == vlan_to_stg_map.end()){
            vlan_to_stg_map[id] = entry->nas_stg_id;
            entry->vlan_list.insert(id);
            NAS_STG_LOG(INFO,3,"Added vlan id %d to default instance",id);
        }else{
            NAS_STG_LOG(INFO,3,"vlan id %d already part of instance %d",id,it->second);
        }
    }else{
        bridge_to_vlan_map.erase(bridge_index);
        auto it = vlan_to_stg_map.find(id);
        if( it == vlan_to_stg_map.end()){
            NAS_STG_LOG(ERR,0,"No STG instance for vlan %d exsist",id);
        }else{
            NAS_STG_LOG(INFO,3,"Removing VLAN %d from Bridge %d",id,bridge_index);
            auto sit = nas_stg_table.find(it->second);
            nas_stg_entry_t * entry = &(sit->second);
            entry->vlan_list.erase(id);
            vlan_to_stg_map.erase(id);
            NAS_STG_LOG(INFO,3,"Deleting VLAN %d from STG instance %d",id,it->second);
        }
    }
...
t_std_error nas_stg_update_vlans(cps_api_object_t obj, nas_stg_id_t id,bool add){
    ...
    if (sit == nas_stg_table.end()){
        NAS_STG_LOG(ERR,0,"No STG Id %d exist to update vlans",id);
        return STD_ERR(STG,NEXIST,0);
    }
    nas_stg_vlan_list_t del_vlan_list;
    cps_api_object_it_t it;
...
static bool nas_stg_lag_set(hal_ifindex_t lag_index, cps_api_object_t obj){
    ...
    if(it == nas_lag_map.end()){
        NAS_STG_LOG(INFO,3,"No Lag Interface Index %d exist in map",lag_index);
        return false;
    }
    ...
    for ( ; cps_api_object_it_valid(&obj_it) ; cps_api_object_it_next(&obj_it) ) {
        ...
        case DELL_IF_IF_INTERFACES_INTERFACE_MEMBER_PORTS:
            ...
            break;
        default :
            NAS_STG_LOG(INFO, 3,"Unknown Attribute %d for LAG",
    ...
    auto lag_stp_it = lag_stp_state_map.find(lag_index);
    /*
     * If existing port list is greater than zero means those ports are not
     * part of the lag anymore so make those ports forwarding using default
     * stg entry
     */
    if(port_list->size() > 0){
        if(lag_stp_it != lag_stp_state_map.end()){
            nas_stg_entry_t * default_entry;
            if(!nas_stg_get_default_entry(&default_entry)){
                NAS_STG_LOG(ERR,0,"Failed to get default STG entry");
                return STD_ERR(STG,FAIL,0);
            }
            NAS_STG_LOG(INFO,3,"Setting STP State to forward for removed lag interface");
            if(!nas_update_stp_state(default_entry,lag_index,BASE_STG_INTERFACE_STATE_FORWARDING)){
                return STD_ERR(STG,FAIL,0);
            }
        }
    }
     ...
     */
    if(lag_stp_it != lag_stp_state_map.end()){
        auto stp_it = lag_to_stp_map.find(lag_index);
        if(stp_it != lag_to_stp_map.end()){
            auto stp_table_it = nas_stg_table.find(stp_it->second);
            if(stp_table_it != nas_stg_table.end()){
                nas_stg_entry_t * entry = &(stp_table_it->second);
                NAS_STG_LOG(INFO,3,"Setting stp state to %d for new lag member",lag_stp_it->second);
...
t_std_error nas_stg_lag_update(hal_ifindex_t lag_index, cps_api_object_t obj){
    ...
    if(op == cps_api_oper_CREATE) {
        if (it == nas_lag_map.end()){
            ...
            NAS_STG_LOG(INFO,3,"Lag Interface Index %d already exist in map",lag_index);
        }
    }
    else if (op == cps_api_oper_DELETE) {
        if (it != nas_lag_map.end()){
            lag_stp_state_map.erase(it->first);
            lag_to_stp_map.erase(it->first);
            nas_lag_map.erase(it);
        }else{
            NAS_STG_LOG(ERR,0,"No Lag Interface Index %d exist in map",lag_index);
            return STD_ERR(STG,NEXIST,0);
        }
    }
