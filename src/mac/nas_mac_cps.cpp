...
static cps_api_return_code_t cps_nas_mac_get_function (void * context, cps_api_get_params_t * param, size_t ix) {
    ...
    if (!cps_api_key_from_attr_with_qual(&key, BASE_MAC_QUERY_OBJ, cps_api_qualifier_TARGET)) {
        NAS_MAC_LOG(ERR, ev_log_s_MINOR, "Key for getting MAC object is not valid");
    ...
    switch (request_type) {
        case BASE_MAC_COMMAND_REQUEST_TYPE_CMD_TYPE_VLAN:
            ...
            NAS_MAC_LOG(ERR, ev_log_s_MINOR, "error getting vlan based macs");
            break;
        ...
        case BASE_MAC_COMMAND_REQUEST_TYPE_CMD_TYPE_INTERFACE:
            ...
            NAS_MAC_LOG(ERR, ev_log_s_MINOR, "error getting interface based macs");
            break;
        case BASE_MAC_COMMAND_REQUEST_TYPE_CMD_TYPE_COUNT:
             ...
             NAS_MAC_LOG(ERR, ev_log_s_MINOR, "error getting count of macs");
             break;
         ...
         default:
             ...
             NAS_MAC_LOG(ERR, ev_log_s_MINOR, "error getting all macs");
             break;
    }
...
static cps_api_return_code_t cps_nas_mac_set_function(void * context, cps_api_transaction_params_t * param, size_t ix) {
    ...
    if( op == cps_api_oper_SET){
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR, "in update/set request handling ");
...
static void nas_mac_event_notification_cb(npu_id_t npu_id, ndi_mac_event_type_t mac_event, ndi_mac_entry_t *mac_entry, bool is_lag_index)
{
    ...
    if (!is_lag_index) {
        ...
        if (dn_hal_get_interface_info(&intf_ctrl) != STD_ERR_OK) {
            NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"NDI MAC Get interface info failed.");
            return;
        }
...
static cps_api_return_code_t nas_mac_flush_handler (void * context,
    ...
    if (op != cps_api_oper_ACTION) {
        EV_LOG(ERR,L2MAC,0,"NAS-MAC","Invalid operation %d for flusing macs",op);
    ...
    if(nas_mac_cps_flush_entry(obj)!= STD_ERR_OK){
        EV_LOG(ERR,L2MAC,0,"NAS-MAC","Failed to flush entries");
...
static cps_api_return_code_t nas_mac_flush_mgmt_set(void * context,
    ...
    if(auto_flush_attr == NULL){
        EV_LOG(ERR,L2MAC,0,"NAS-MAC","No value passed to change auto mac management");
        return (cps_api_return_code_t)STD_ERR(MAC,FAIL,0);
    }
    ...
    EV_LOG(INFO,L2MAC,0,"NAS-MAC","Auto MAC management value set to %d",mac_auto_flush);
...
static cps_api_return_code_t nas_mac_flush_mgmt_get(void * context, cps_api_get_params_t * param, size_t ix){
    ...
    if (obj == NULL) {
        EV_LOG(ERR,L2MAC, 0,"NAS-MAC","Failed to create/append new object to list");
...
static t_std_error cps_nas_mac_init(cps_api_operation_handle_t handle) {
    ...
    if (!cps_api_key_from_attr_with_qual(&f.key, BASE_MAC_TABLE_OBJ, cps_api_qualifier_TARGET)) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not translate %d to key %s,", (int)(BASE_MAC_TABLE_OBJ),
                cps_api_key_print(&f.key, buff, sizeof(buff)-1));
        return STD_ERR(MAC,FAIL,0);
    }

    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Registering for BASE_MAC_TABLE_OBJ %s",
            cps_api_key_print(&f.key,buff,sizeof(buff)-1));
    ...
    if (!cps_api_key_from_attr_with_qual(&f.key, BASE_MAC_QUERY_OBJ, cps_api_qualifier_TARGET)) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not translate %d to key %s,", (int)(BASE_MAC_QUERY_OBJ),
                cps_api_key_print(&f.key, buff, sizeof(buff)-1));
        return STD_ERR(MAC,FAIL,0);
    }

    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Registering for BASE_MAC_QUERY_OBJ %s",
            cps_api_key_print(&f.key,buff,sizeof(buff)-1));
    ...
    if (!cps_api_key_from_attr_with_qual(&f.key,BASE_MAC_FLUSH_OBJ,
                                             cps_api_qualifier_TARGET)) {
        NAS_MAC_LOG(TRACE, ev_log_s_MAJOR,"Could not translate %d to key %s",
...
static cps_api_return_code_t nas_mac_vlan_process_port_membership(cps_api_object_t obj, bool add_ports)
{
    ...
    if (add_ports) {
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"NAS Vlan port update for add, nothing to do, returning");
        return cps_api_ret_code_OK;
    }

    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"NAS port vlan membership update");

    cps_api_object_attr_t vlan_attr = cps_api_object_attr_get(obj, DELL_BASE_IF_CMN_IF_INTERFACES_INTERFACE_IF_INDEX);

    if(vlan_attr == NULL) {
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Missing ifindex attribute for CPS Set");
        return cps_api_ret_code_ERR;
    }
    ...
    for ( ; cps_api_object_it_valid(&it) ; cps_api_object_it_next(&it) ) {
        ...
        if(vlan_id_attr == NULL) {
            NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Missing Vlan ID for CPS Set");
            return cps_api_ret_code_ERR;
        }

        vlan_id = (uint32_t) cps_api_object_attr_data_u32(vlan_id_attr);

        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Vlan index 0x%x", vlan_id);
        if (!vlan_id) {
            NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Invalid Vlan index 0x%x, returning", vlan_id);
            return cps_api_ret_code_ERR;
        }
        ...
        if((port_list_attr == NULL) && (tag_port_list_attr == NULL)) {
           NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Missing Port list for CPS Set");
           return cps_api_ret_code_ERR;
        }
        ...
        if (port_list_attr) {
            ...
            NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"untagged port index 0x%x", port_index);
        ...
        if (tag_port_list_attr) {
            ...
            NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"tagged port index 0x%x", port_index);
...
{
    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Received VLAN Port notification");

    if(!mac_auto_flush){
        NAS_MAC_LOG(TRACE, 0,"NAS MAC auto flush management disabled,nothing to do");
        return true;
    }
    cps_api_operation_types_t op = cps_api_object_type_operation(cps_api_object_key(obj));
    ...
    if (op == cps_api_oper_CREATE) {
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Received ADD VLAN operation code.. ");
        add_ports = true;
    } else if (op == cps_api_oper_DELETE) {
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Received DEL VLAN operation code.. ");
    } else {
        NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Received Invalid VLAN operation code, 0x%x", op);
        return cps_api_ret_code_ERR;
    }
    ...
    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"Port vlan membership event processing done .. ");
    return cps_api_ret_code_OK;
}

static bool nas_mac_if_event_cb(cps_api_object_t obj, void *param)
{
    NAS_MAC_LOG(INFO,3,"Interface event handling");
    ...
    if(oper_status == IF_INTERFACES_STATE_INTERFACE_OPER_STATUS_DOWN){
        ...
        if(nas_mac_handle_if_down(index) != STD_ERR_OK){
            NAS_MAC_LOG(ERR,0,"Flush on interface %d failed when it went oper down",index);
            return false;
        }
    }
    NAS_MAC_LOG(INFO,3,"Flushed Dynamic Entries on an interface %d",index);
...
t_std_error nas_mac_reg_if_event (void) {
    ...
    if (cps_api_event_thread_reg(&reg, nas_mac_if_event_cb,NULL)!=cps_api_ret_code_OK) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not register for if events");
        return STD_ERR(MAC,FAIL,0);
    }
    return STD_ERR_OK;
}
...
{
    NAS_MAC_LOG(INFO,3,"LAG event handling");
    ...
    if(admin_status == IF_INTERFACES_STATE_INTERFACE_ADMIN_STATUS_DOWN){
        std_mutex_simple_lock_guard lock(&nas_mac_mutex);
        if(nas_mac_handle_if_down(index) != STD_ERR_OK){
            NAS_MAC_LOG(ERR,0,"Flush on LAG interface %d failed when it went oper down",index);
            return false;
        }
        NAS_MAC_LOG(INFO,3,"Flushed Dynamic Entries on LAG interface %d",index);
    }
...
t_std_error nas_mac_reg_lag_event (void) {
    ...
    if (cps_api_event_thread_reg(&reg, nas_mac_lag_event_cb,NULL)!=cps_api_ret_code_OK) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not register for lag events");
        return STD_ERR(MAC,FAIL,0);
    }
    return STD_ERR_OK;
}
...
t_std_error nas_mac_reg_vlan_event (void) {
    ...
    if (cps_api_event_service_init() != cps_api_ret_code_OK) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not initialize the event service");
        return STD_ERR(MAC,FAIL,0);
    }

    if (cps_api_event_thread_init() != cps_api_ret_code_OK) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not initialize the event thread");
        return STD_ERR(MAC,FAIL,0);
    }
    ...
    if (cps_api_event_thread_reg(&reg, nas_mac_vlan_event_cb,NULL)!=cps_api_ret_code_OK) {
        NAS_MAC_LOG(ERR, ev_log_s_MAJOR,"Could not register for vlan events");
        return STD_ERR(MAC,FAIL,0);
    }
...
t_std_error nas_mac_init(cps_api_operation_handle_t handle) {
    ...
    if (std_thread_create(&nas_l2_mac_req_handler_thr)!=STD_ERR_OK) {
        NAS_MAC_LOG(ERR, 0, "Error creating nas mac request thread");
        return STD_ERR(MAC,FAIL,0);
    }

    NAS_MAC_LOG(TRACE, ev_log_s_MINOR,"nas_mac_init: SUCCESS");
