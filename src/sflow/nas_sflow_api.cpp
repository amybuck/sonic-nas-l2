...
static bool nas_validate_sflow_entry(nas_sflow_entry_t *entry){
    ...
    if(!entry->attr_set.contains(BASE_SFLOW_ENTRY_DIRECTION)){
        NAS_SFLOW_LOG(ERR,ev_log_s_MINOR,"Missing SFLOW_ENTRY_TABLE_DIRECTION Parameter"
                                           "for creating sFlow session");
        return false;
    }

    if(!entry->attr_set.contains(BASE_SFLOW_ENTRY_SAMPLING_RATE)){
        NAS_SFLOW_LOG(ERR,ev_log_s_MINOR,"Missing SFLOW_ENTRY_TABLE_SAMPLING_RATE Parameter"
                                    "for creating sFlow session");
        return false;
    }
    return true;
...
t_std_error nas_sflow_delete_session(nas_sflow_id_t id){
    ...
    if(!nas_sflow_update_direction(entry,false)){
        NAS_SFLOW_LOG(ERR,0,"Failed to removed source ports from session %d",
    ...
    NAS_SFLOW_LOG(INFO,0,"Deleted nas sflow entry with id %d",it->second.nas_sflow_id);
