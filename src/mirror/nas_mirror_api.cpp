...
static bool nas_mirror_update_src_port(nas_mirror_entry * entry,hal_ifindex_t ifindex,
    ...
    else if(port.direction == BASE_CMN_TRAFFIC_PATH_EGRESS && old_dir == BASE_CMN_TRAFFIC_PATH_INGRESS){
        if(ndi_mirror_update_direction(entry->get_ndi_entry(),port, true)!=STD_ERR_OK) return false;
        port.direction = old_dir;
        if(ndi_mirror_update_direction(entry->get_ndi_entry(),port, false)!=STD_ERR_OK) return false;
    }
...
static inline bool nas_mirror_validate_attr(nas::attr_set_t &attrs){
    ...
    if(!((attrs.contains(BASE_MIRROR_ENTRY_DST_INTF) ||
        ...
        NAS_MIRROR_LOG(ERR,ev_log_s_MINOR,"Missing Necessary Parameters for creating a Mirroring Session");
        return false;
    }
    return true;
}
...
static inline bool nas_mirror_validate_erspan_attr(nas::attr_set_t &attrs){
    ...
    if(!(attrs.contains(BASE_MIRROR_ENTRY_SOURCE_IP) && attrs.contains(BASE_MIRROR_ENTRY_DESTINATION_IP) &&
        ...
        NAS_MIRROR_LOG(ERR,ev_log_s_MINOR,"Missing Necessary Parameters for creating a ERSPAN Mirroring Session");
        return false;
    }
    return true;
}
...
t_std_error nas_mirror_delete_session(cps_api_object_t obj,nas_mirror_id_t id) {
    ...
    if(it == nas_mirror_table.end()){
        NAS_MIRROR_LOG(ERR,ev_log_s_MAJOR,"No Mirror oid %d exist",(int)id);
        return STD_ERR(MIRROR,NEXIST,0);
    }

    nas_mirror_entry & entry = it->second;
    if(!entry.remove_src_intf()){
        NAS_MIRROR_LOG(ERR,0,"Failed to remove source ports for Mirror Session %d",
                entry.get_id());
        return STD_ERR(MIRROR,FAIL,0);
    }
    ...
    NAS_MIRROR_LOG(INFO,3,"Deleted Mirror Session Id %d",entry.get_id());
